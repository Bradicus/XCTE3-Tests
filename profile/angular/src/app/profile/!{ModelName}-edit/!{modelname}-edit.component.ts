...
export class !{ModelName}EditComponent implements OnInit  {
    ...
    onSubmit() {
        if (!this.!{ModelName}EditForm.invalid) {
            if (this.!{ModelName}EditForm.controls['id'].value === null || !(this.!{ModelName}EditForm.controls['id'].value > 0)) {
                this.profileUserDataStoreService.create(this.!{ModelName}EditForm.value).subscribe(newItem =>  {
                    this.item = newItem;
                });
            } else {
            
                this.profileUserDataStoreService.update(this.!{ModelName}EditForm.value).subscribe(newItem =>  {
                    this.item = newItem;
                });
            }
        }
    ...
    populate(): void {
        this.profileUserDataMapService.populate(this.!{ModelName}EditForm as FormGroup, this.item);
    }
}
